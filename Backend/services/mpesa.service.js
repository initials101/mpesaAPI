...
try {
// Save to database
const savedTransaction = await fine.table("transactions").insert(transaction);
logger.info(`Transaction saved to database with ID: ${savedTransaction[0]?.id || 'unknown'}`);
} catch (dbError) {
...
logger.info(`Transaction ${checkoutRequestID} status updated to ${updatedTransactions[0].status}, stopping polling`);
return;
}
} catch (error) {
...
logger.error(`Error updating transaction ${id} status:`, updateError);
}
} else {
...
logger.error(`Error updating transaction ${id} timeout handled flag:`, updateError);
}
}
} catch (error) {
...
try {
const updatedTransactions = await fine.table("transactions").select().eq("id", transaction.id);
return updatedTransactions[0];
} catch (fetchError) {
logger.error(`Error fetching updated transaction ${transaction.id}:`, fetchError);
return null;
}
} catch (error) {
...
try {
const savedTransaction = await fine.table("transactions").insert(transaction);
logger.info(`B2C transaction saved to database with ID: ${savedTransaction[0]?.id || 'unknown'}`);
} catch (dbError) {
...
// Save to database
const savedTransaction = await fine.table("transactions").insert(transaction);
logger.info(`Failed B2C transaction saved to database with ID: ${savedTransaction[0]?.id || 'unknown'}`);
} catch (dbError) {
